...
| Paste `GITHUB_TOKEN` into `.env.local` | **You** | Fine-grained PAT with **Issues write** on the quacker repo (Edge suggestion issues only — not git push). Agent runs `scripts/setup-suggestion-github-webhook.sh`. Local push/PR uses `gh auth login`, not this token. |
