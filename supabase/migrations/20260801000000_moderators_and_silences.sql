-- Room moderators + persistent silence
--
-- 1. Extend group_role with 'mod'.
-- 2. group_silences persists mute across leave/rejoin (not on membership).
-- 3. Staff (creator or mod) may rename rooms and manage silence/mod.
-- 4. Drop creator kick (ban); membership only via self-leave or room delete.
...
create table public.group_silences (
...
create index group_silences_user_id_idx on public.group_silences (user_id);
...
-- Staff may update other members' rows (role); self still covered by update_self
...
-- Upsert path may UPDATE snapshot fields
